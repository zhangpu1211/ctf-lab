...
- `Info.plist`、MANIFEST 与 Mach-O 最低版本统一声明 macOS 26.0；macOS 15 兼容构建只保留为历史
  验证证据，不再作为当前产品的支持承诺；
- 不改 `stop/reset` 行为；`run` 的默认策略是 Kali user-mode NAT + SPICE 真实自动分辨率，其他节点
  Cocoa + 管理网隔离；`auto` 缺失完整 SPICE 能力时退化为 Cocoa，显式 `--display spice` 缺件不回退；
  不动 UTM 路径；
...
- **不做第二套逻辑**：GUI 只调用既有 CLI（`dist verify --json`、`import <profile> <基盘> --manifest`、
  ...
  另以固定动作调用 `run` 的单节点选择；Kali 的联网与 SPICE 自动分辨率/Cocoa 退化由 CLI 默认策略统一处理，
  不要求 GUI 自己拼接显示参数，
