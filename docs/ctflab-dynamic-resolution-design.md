...
> 状态：**设计文档，未实现任何后端**。当前默认显示路径（Homebrew QEMU + Cocoa + `zoom-to-fit`）
> 不支持动态分辨率；固定 `xres/yres` 与窗口缩放也都不是动态分辨率。本文只做真实路径的对比、
> 边界定义与决策请求，不包含可执行代码，也不改变现有 CLI 行为与默认 QEMU 命令。
...
- 新增一次性导出：从已导入的只读基础镜像生成一个**全新的 `<name>.utm` 包**；
- 不修改、不覆盖、不重命名用户已有的任何 UTM 虚拟机；只做只读枚举以避免重名；
- 首版**不接受任意用户 `.utm` 模板**：配置来自版本固定、哈希校验的内置 fixture，或从严格
  结构化白名单重建；禁止共享目录、额外 QEMU 参数、共享/桥接网络与端口转发，越界字段一律
  拒绝导出，绝不透传；
- 输出边界：目标路径已存在（文件或目录）一律拒绝，没有强制覆盖，也没有
  “本工具生成标记可复用”的例外；
- 包内显示参数固定为现场 PoC 组合：启用动态分辨率 + `virtio-gpu-pci`；
...
- 基础镜像：只读；导出前记录 SHA-256，导出完成后复核未变化；绝不写入 `images/` 目录；
- 包内磁盘：`qemu-img convert` 生成的独立副本，可写、可丢弃；不存在 CTFLab overlay 语义，
  每次重新导出即回到初始状态；
...
### 2.3 CLI/API 草案（未实现，不是当前 CLI 的一部分）
...
inputs:
  - 已导入的只读基础镜像（按 profile）
  - 内置 UTM 配置 fixture：版本固定、SHA-256 随发布登记；不接受用户提供的模板
outputs:
  - <name>.utm/              # 新包；目标路径已存在时拒绝，不覆盖
  - <name>.utm.export.json   # 来源镜像 SHA-256、fixture 版本/哈希、离线基盘前提记录、导出时间、UTM 版本
checks:
  - 来源镜像存在且哈希与登记值一致
  - profile 为 aarch64 + virtio 磁盘/显示；其余架构首版明确拒绝并说明原因
  - fixture 哈希与登记值一致，生成配置只允许白名单键（无 Network、无 Clipboard Sharing/共享目录、无附加 QEMU 参数）
  - 目标路径已存在（文件或目录）一律拒绝；无强制覆盖选项，也无“工具生成标记可复用”例外
...
1. 固定内置 UTM 配置 fixture（版本 + SHA-256 随发布登记，内容取自人工核对过的 PoC 配置），
   实现严格键白名单重建：只写入允许的键，禁止自由文本注入与任何字段透传；
2. `utm-export`：`qemu-img convert` 生成包内独立磁盘 + 导出清单 + 输出覆盖保护
   （任何已存在路径一律拒绝）；
3. 单元测试：fixture 哈希校验、白名单重建、拒绝任何已存在输出（文件/目录）、来源哈希前后不变、
   非 aarch64 拒绝、导出清单字段完整、生成配置无网络/无共享目录/无附加参数；
4. 离线基盘前提记录：只登记离线可核验的基盘事实，不得声称图形会话 agent 已连接（见 2.1）；
5. 人工 E2E 验收清单（在导出包启动后执行，写入验证记录才算交付）：UTM 打开包 → 启动 →
   来宾图形会话中 `spice-vdagent` 进程运行且 agent 已连接 → 至少两种窗口尺寸下实际
   `xrandr --current` 模式跟随（这是动态分辨率的唯一证据）→ 宿主/来宾剪贴板双向负向
   （英文、中文、多行哨兵均不泄漏，Clipboard Sharing 已关闭）→ 网络三层隔离
   （无 WAN 出口、无物理 LAN 可达、无法访问 CTFLab 实验网）→ 关闭并删除包后无残留。
   只有上述证据记录在案，才允许对外称“动态分辨率就绪”；仅有 `zoom-to-fit`、仅
...
state:
  ...
  spice_endpoint_unix: unix:<runtime_dir>/spice.sock   # 优先；目录 0700；不要求 addr=
...
0. 前置 PoC：自管 QEMU + SPICE 的图形设备与客户端组合验证（UTM 的 `virtio-gpu-pci` 结果
   不能直接套用）；未通过前不进入以下实现；
1. 能力探测函数 + 单元测试（伪造 QEMU 输出：支持 / 不支持 / 命令异常三态）；
2. 显示后端选择与命令拼装（本机端点 + 每次运行本地鉴权、始终保留唯一 virtio-serial +
   spicevmc agent transport、剪贴板标志随 `--clipboard` 切换：未授权 `disable-copy-paste=on`、
   授权 `disable-copy-paste=off`，`disable-agent-file-xfer=on` 保留、仅 `kali-arm64`）；
3. 显式请求失败语义（启动前报错、不自动回退）与状态记录（缺失项与原因在 `status` 可见）；
4. 隔离验证：agent transport 存在时的三层剪贴板负向验证（英文、中文、多行双向）、
   未授权本地客户端无法连接；任一层不能证明隔离，路径 B 整体不实现；
5. 默认路径回归（不传选项时命令快照与现状一致）；
6. 手工 E2E 清单（见第 5 节），通过前不得对外宣称支持动态分辨率。

## 4. 路径对比

| 维度 | A：独立 UTM 导出 | B：带 SPICE 的 QEMU + 本地客户端 |
|---|---|---|
| 动态分辨率证据 | UTM 显示机制 PoC 已验证（2026-09-14）；CTFLab 导出包尚未 E2E 验收 | 本机未实测，需先做 PoC |
...
| B：UNIX socket 模式 | 命令含 `unix=<0700 目录下 socket>`，且不含 `addr=`/`port=`（不把 TCP 要求混入该模式） |
...
| B（正向：UNIX socket 模式） | 客户端经 `unix:<runtime_dir>/spice.sock` 连接（目录 0700、无 TCP 监听）→ 窗口缩放/全屏时 xrandr 跟随 → `--clipboard` 时英文/中文/多行文本双向逐字节一致 → `stop --graceful` 回归 → 默认 Cocoa 与 `--headless` 回归 |
| B（正向：TCP 模式回退） | 客户端经 `127.0.0.1:<受控端口>` + 每次运行鉴权连接 → 窗口缩放/全屏时 xrandr 跟随 → `lsof -iTCP` 无非回环监听、无未授权连接 → `--clipboard` 时英文/中文/多行文本双向逐字节一致 → `stop --graceful` 回归 |
...
本次完整回归（2026-09-14，95 项）必须全部通过。

## 6. 推荐路径

- **近期（小切片，可立即决策）**：先做路径 A 的最小实现切片，定位为“显示体验导出”能力：
  复用已通过 PoC 的 UTM 显示机制（CTFLab 导出的 UTM 包仍需按 2.4 做 E2E 验收），
  不动运行器、不动默认 QEMU 命令；导出包默认无网卡，不接入实验网。
- **中期（依赖 Task 6）**：路径 B 以独立 PoC 为前置门槛（自管 QEMU + SPICE 的图形设备与
  客户端组合，UTM 的 `virtio-gpu-pci` 结果不套用），再做能力探测、本地鉴权与三层剪贴板
  负向 E2E；在全部通过前，显示后端不进入默认路径，文档与 CLI 都不得宣称 CTFLab 支持动态分辨率。
- **两条路径的共同底线**：默认 Cocoa 与 `zoom-to-fit` 现状不变；
  “动态分辨率已支持”的声明只在对应路径 E2E 验收记录之后给出。
...
- 不允许路径 B 用于 `kali-arm64` 之外的 profile；不把 UTM 的 `virtio-gpu-pci` 结果直接
  套用到自管 QEMU + SPICE（独立 PoC 未通过前不实现）；
- 不在 Task 6 打包完成前分发或内置 QEMU 与 SPICE 客户端二进制；
- 不允许 SPICE 以非本机方式暴露：TCP 仅 `127.0.0.1` + 每次运行鉴权，UNIX socket 仅
  0700 runtime 目录；不以 VNC 作为默认显示路径；
- 本次未新增动态分辨率相关 CLI、`run --display` 或 SPICE 显示参数（工作区已有的
  `probe --matrix` 等参数不属于显示功能）；
- 本轮不实现 UTM 后端与 SPICE 后端（本文只是设计）。
